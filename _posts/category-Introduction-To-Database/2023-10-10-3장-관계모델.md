...
　한 릴레이션의 **인스턴스**(instance)는 **레코드**(record)라고 하는 **투플**(tuple)들의 집합이다. 각 투플들은 그 릴레시션 스키마와 동일한 수의 필드를 가지고 있다.
...
　Students 릴레이션에 추가하여, 다음과 같은 두 번째 릴레이셔이 있다고 하자.
```
...
```
　외래키 제약조건은 Enrolled 릴레이션에 있는 모든 studid 값이 Students 릴레이션에 반드시 나타나야 한다는 것을 선언한다. 즉, Entolled에 있는 studid 값이 Students 릴레이션에 반드시 나타나야 한다는 것을 선언한다. 즉, Enrolled에 있는 studid는 Students를 참조하는 외래키이다.
